...
async function doBatchFromScratch({
	...
	if( modelParams == null || modelParams=== undefined ) throw 'Model params is null';

    let model = deepmodels.getDeepModel(modelParams);
    math = model.math;

    return await math.scope(async () => {

        optimizer = deepmodels.getOptimizer(optimizerType, learningRate, momentum);
	    ...
	    weightInit = weightInit.map( (e) => Array.prototype.slice.call(e) )

	    optimizerParams = getOptimizerParams(optimizer, data);

	    let optimizerParamsToArray = {};
	    for (let [key, value] of Object.entries(optimizerParams)) {
	    	optimizerParamsToArray[key] = Array.prototype.slice.call(value)
	    }


	    // here you should jusrt return arrays nothing more
        return([weightInit, optimizerParamsToArray, costVal]);
    })
}
